...
if ($whatInfo == "amendRunner") {
    ...
    $txtRunnerTelNo = htmlspecialchars($_POST["txtRunnerTelNo"]);
    $txtRunnerFirstLineAddress = htmlspecialchars($_POST["txtRunnerFirstLineAddress"]);
    $txtRunnerPostcode = htmlspecialchars($_POST["txtRunnerPostcode"]);
    $txtRunnerEmail = htmlspecialchars($_POST["txtRunnerEmail"]);
    ...
    $sql = "UPDATE tblRunners SET RunnerFirstName='" . $txtRunnerFirstName .
        ...
        "', RunnerPhoneNo='" . $txtRunnerTelNo .
        "', RunnerAddressFirstLine='" . $txtRunnerFirstLineAddress .
        "', RunnerAddressPostcode='" . $txtRunnerPostcode .
        "', RunnerEmail='" . $txtRunnerEmail .
...
if ($whatInfo == "addRunner") {
    ...
    $sql = "INSERT INTO tblRunners (RunnerFirstName, RunnerSurname, RunnerDiv, RunnerSubsPaid, RunnerDOB, RunnerSex, RunnerPhoneNo, RunnerAddressFirstLine, RunnerAddressPostcode, RunnerEmail) " .
...
// NEW WMA ENTRY CODE - START
        ...
        $sqlCheckForWMATimeEntry = "SELECT tblWMARaceTimes.RaceID FROM tblWMARaceTimes WHERE tblWMARaceTimes.RaceID=$txtRaceID[0] AND tblWMARaceTimes.RunnerID = $txtRunnerID[0]";
        ...
        if($resultWMACheck->num_rows > 0)
        {
            ...
            SELECT tblRaceTimes.RaceTime FROM tblRunners JOIN tblRaceTimes ON tblRunners.RunnerID = tblRaceTimes.RunnerID WHERE RaceID='" . $row['RaceID'] . "' AND RunnerID='" . $row['RunnerID'] . "') WHERE RaceID='" . $row['RaceID'] . "' AND RunnerID='" . $row['RunnerID'] . "';
...
function rankOpenChampWMAMRace($RID) {    //
    ...
    // the second SELECT statement is important as it returns the results of the stored procedure //
    // OLD $sqlRankWMAM = sprintf('CALL proc_openChamp_WMA_M_rank(%1$s,%2$s); SELECT WMARaceTimesID, RaceTime, WMARaceTime;',date("Y") . '-01-01', $RID);
