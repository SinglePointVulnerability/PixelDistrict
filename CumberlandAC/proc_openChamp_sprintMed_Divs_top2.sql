...
(SELECT OpenChampDivGenRacePoints
 ...
 (Select RaceID
  FROM tblRaces
  WHERE RaceCode = 32 AND ChampYear = champ_year)
 ORDER BY OpenChampDivGenRacePoints DESC LIMIT 2) overall);

IF record_exists_mf_id IS NOT NULL THEN
UPDATE tblOpenChampDivGenOverallPoints SET OpenChampCatSprintMedDivGenOverallPoints = total_points_mf, RunnerSex = runner_sex, RunnerDiv = runner_div WHERE OpenChampDivGenOverallPointsID = record_exists_mf_id; COMMIT;
ELSE
