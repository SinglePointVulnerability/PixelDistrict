...
(SELECT OpenChampDivGenRacePoints
 ...
 (Select RaceID
  FROM tblRaces
  WHERE RaceCode IN (2) AND ChampYear = champ_year)
 ORDER BY OpenChampDivGenRacePoints DESC LIMIT 2) overall);

IF record_exists_mf_id IS NOT NULL THEN
UPDATE tblOpenChampDivGenOverallPoints SET OpenChampCatMidDivGenOverallPoints = total_points_mf, RunnerSex = runner_sex, RunnerDiv = runner_div WHERE OpenChampDivGenOverallPointsID = record_exists_mf_id; COMMIT;
ELSE
INSERT INTO tblOpenChampDivGenOverallPoints (RunnerID, RunnerSex, RunnerDiv, OpenChampCatMidDivGenOverallPoints, ChampionshipID, ChampYear) VALUES (runner_id, runner_sex, runner_div, total_points_mf, 1, champ_year); COMMIT;
END IF;
...
UPDATE tblOpenChampDivGenOverallPoints SET OpenChampCatMidDivGenOverallPoints = total_points_joint, RunnerSex = 'Joint', RunnerDiv = runner_div WHERE OpenChampDivGenOverallPointsID = record_exists_joint_id; COMMIT;
ELSE
INSERT INTO tblOpenChampDivGenOverallPoints (RunnerID, RunnerSex, RunnerDiv, OpenChampCatMidDivGenOverallPoints, ChampionshipID, ChampYear) VALUES (runner_id, 'Joint', runner_div, total_points_joint, 1, champ_year); COMMIT;
END IF;
*/
END