<?php
    ...
    $ChampionshipName = "Ladies Open Championship";
...
/*    $sqlGetOpenChampLadiesRunners = "select DISTINCT tblOpenChampLadiesOverallPoints.RunnerID, tblRunners.RunnerFirstName, tblRunners.RunnerSurname, tblOpenChampLadiesOverallPoints.OpenChampLadiesOverallPoints, tblOpenChampLadiesOverallPoints.ChampionshipID, tblOpenChampLadiesOverallPoints.ChampYear FROM tblOpenChampLadiesOverallPoints INNER JOIN tblRunners ON tblOpenChampLadiesOverallPoints.RunnerID = tblRunners.RunnerID WHERE tblOpenChampLadiesOverallPoints.OpenChampLadiesOverallPoints > 0 AND tblOpenChampLadiesOverallPoints.ChampYear = $RaceYear ORDER BY tblOpenChampLadiesOverallPoints.OpenChampLadiesOverallPoints DESC";*/
    
    if($RaceYear == date("Y")) {
        $sqlGetOpenChampLadiesRunners = "SELECT * FROM viewopenchampladiesrunners_currentyear WHERE ChampYear = $RaceYear ORDER BY OpenChampLadiesOverallPoints DESC";        
    }
    else {
        $sqlGetOpenChampLadiesRunners = "SELECT * FROM viewopenchampladiesrunners WHERE ChampYear = $RaceYear ORDER BY OpenChampLadiesOverallPoints DESC";
    }
...
echo "<th class = \"tblTimesPointsOuterSpacer\"></th>";
    ...
    echo "<button class=\"accordion\">Ladies Open Championship</button><div class=\"panel\">";
    echo "<table id=\"innerTableLadiesOpen\" class='tblTimesPointsInner'>"; // inner table to contain collapsible ladies division content

    // Get the runner details
    if(mysqli_multi_query($conn,$sqlGetOpenChampLadiesRunners))
    {
        do{
            if($result=mysqli_store_result($conn)){
                while($row=mysqli_fetch_assoc($result)){
                    ...
                    $ChampPoints = $row['OpenChampLadiesOverallPoints'];
        ...
        echo "Oops! I couldn't find any Ladies Open Championship runners for $RaceYear!";
    }
    echo "</table></div></td></tr></div>"; // inner table to contain collapsible ladies division content
    echo "</table>";
    echo "<br>";
...
function splitRunnerTimePoints($RaceIDTimePoints){
    ...
    if(strlen($RaceIDTimePoints) < 5) {
        return "<td class=\"tblTimesPointsInnerLadies\"></td>\n";
    }
    else {
        ...
        if($champDisplay == "times") {
            return "<td class=\"tblTimesPointsInnerLadies\"><div class=\"fontRaceTime\" style='display:inline'>$Pieces[1]</div><div class=\"fontRacePoints\" style='display:none'>$Pieces[2]</div></td>\n";            
        }
        else if($champDisplay == "points") {
            return "<td class=\"tblTimesPointsInnerLadies\"><div class=\"fontRaceTime\" style='display:none'>$Pieces[1]</div><div class=\"fontRacePoints\" style='display:inline'>$Pieces[2]</div></td>\n";
        }
    }
}
